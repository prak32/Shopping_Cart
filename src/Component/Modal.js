      ...
      {orderFormVisible && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex justify-center items-center">
          <div className="bg-white p-5 rounded-lg shadow-lg w-2/3 relative">
            ...
            {toast.show && (
              <div className={`absolute top-0 mt-10 ml-8 w-40 px-4 py-2 rounded text-xs md:text-sm md:ml-96 border border-red-900 ${toast.type === 'error' ? 'bg-red-500' : 'bg-green-500'} text-white`}>
                {toast.message}
              </div>
            )}
            <form>
